...
# -------------------- MCMC posterior + corner-plot settings --------------------
# After the deterministic least-squares fit (which seeds the walkers and sets the
# noise levels), sample the joint posterior of the 5 parameters
# [log10(kappa), h_ml, eps, F_ref, lambda] with emcee and render a `corner` plot
# per model, plus a printed "is each parameter well constrained" check.
#
# COST WARNING: every MCMC likelihood evaluation is ONE PDE solve. The number of
# solves per (model, run_type, results) is roughly MCMC_N_WALKERS * MCMC_N_STEPS
# (e.g. 24*800 ~ 19k); across 8 models x 6 run_type/results combos that is ~1e6
# PDE solves and can take HOURS even parallelized over the pool. Smoke-test with
# tiny values first, then scale MCMC_N_STEPS up and check emcee's autocorrelation
# time for convergence. Set RUN_MCMC_CORNER = False to skip it entirely, or narrow
# RUN_TYPES / RESULT_KINDS above to a single combo.
RUN_MCMC_CORNER = True
MCMC_N_WALKERS = 24
MCMC_N_STEPS = 800
MCMC_BURN = 200                  # steps discarded as burn-in
MCMC_THIN = 5                    # keep every MCMC_THIN-th step after burn-in
MCMC_WIDTH_THRESH = 0.5          # "constrained" if 0.5*(p84-p16)/|p50| < this
MCMC_CORR_THRESH = 0.9           # "degenerate" pair if |Pearson corr| > this
# Uniform-prior bounds for [log10(kappa), h_ml, eps, F_ref, lambda] (a superset of
# the curve_fit bounds so the walkers can explore the tails of the posterior).
MCMC_PRIOR_LO = np.array([np.log10(1e-7), 1e-3, 0.0, 0.1, 0.05])
MCMC_PRIOR_HI = np.array([np.log10(1e-3), 300.0, 3.0, 20.0, 5.0])
MCMC_LABELS = [r"$\kappa$", r"$h_{ml}$", r"$\epsilon$", r"$F_{ref}$", r"$\lambda$"]
...
def pde_H_at(t_years, kappa, h_ml, eps_, F_ref_, T_eq_, t_final_yrs_,
             dz_target_=PDE_FIT_DZ_TARGET, rtol=None, atol=None):
...
# emcee samples the joint posterior of theta = [log10(kappa), h_ml, eps, F_ref,
# lambda]; corner renders it. These are module level so emcee can dispatch the
# log-prob across the fork pool (theta + a dict of picklable numpy arrays/floats).
def _mcmc_forward(theta, t_fit_years, reg_years, bin_H, t_final_years):
   """One PDE solve for an MCMC sample. Returns the two observables the
   likelihood needs: T_model at the fit years, and the ocean heat uptake H at the
   regression years (50-yr-binned when bin_H, matching the LS regression rows).
   Uses the loosened sensitivity-sweep solver tolerance -- MCMC wants many cheap
   evaluations, not the tight-tolerance main fit."""
   log10_kappa, h_ml, eps, F_ref, lam = theta
   kappa = 10.0 ** log10_kappa
   T_eq = F_ref / lam
   z_max, nz = _pde_grid(kappa, t_final_years, PDE_FIT_DZ_TARGET)
   pde_p = PDEParams(kappa=kappa, h_ml=h_ml, dT_eq=T_eq, F0=F_ref, epsilon=eps,
                     z_max=z_max, Nz=nz, t_final=t_final_years * YEAR)
   sol = pde_solve_model(
      pde_p, t_eval=_log_spaced_t_eval(pde_p.t_final, SENSITIVITY_N_SAVE),
      rtol=SENSITIVITY_RTOL, atol=SENSITIVITY_ATOL,
   ...
   t_yr = sol["t"] / YEAR
   T_model = np.interp(t_fit_years, t_yr, sol["dT"])
   # H = -D*d(theta)/dz|_0 via the same 2nd-order one-sided stencil as pde_H_at.
   theta_z, dz = sol["theta"], sol["dz"]
   dtheta_dz0 = (-3.0 * theta_z[0] + 4.0 * theta_z[1] - theta_z[2]) / (2.0 * dz)
   H_series = -pde_p.D * dtheta_dz0
   H = np.interp(reg_years, t_yr, H_series)
   if bin_H:
      H = np.array([H[i:i + 50].mean() for i in range(50, H.shape[0], 50)])
   return T_model, H


def _mcmc_log_prob(theta, data):
   """Log posterior for theta = [log10(kappa), h_ml, eps, F_ref, lambda].
   Uniform priors (bounds in `data`) + Gaussian likelihood that jointly fits the
   AOGCM temperature timeseries AND the Net TOA regression (so all 5 parameters
   are identified): N_model = F - lambda*T_reg - (eps-1)*H."""
   theta = np.asarray(theta, dtype=float)
   if np.any(theta < data["prior_lo"]) or np.any(theta > data["prior_hi"]):
      return -np.inf
   try:
      T_model, H = _mcmc_forward(theta, data["t_fit"], data["reg_years"],
                                 data["bin_H"], data["t_final_years"])
   except Exception:
      return -np.inf
   if not (np.all(np.isfinite(T_model)) and np.all(np.isfinite(H))):
      return -np.inf
   _, _, eps, F_ref, lam = theta
   N_model = F_ref - lam * data["T_reg"] - (eps - 1.0) * H
   chi2_T = np.sum((T_model - data["T_fit_abs"]) ** 2) / data["sigma_T"] ** 2
   chi2_N = np.sum((N_model - data["N_reg"]) ** 2) / data["sigma_N"] ** 2
   return -0.5 * (chi2_T + chi2_N)


def run_mcmc_corner(theta_hat, data, model, n_samples_label, png_path, pdf_path):
   """Sample the posterior around the LS best fit theta_hat and save a corner
   plot (kappa shown on a linear axis after 10**log10). Returns a summary dict
   (16/50/84 percentiles in display units, the flat display chain, acceptance)."""
   ndim = 5
   lo, hi = data["prior_lo"], data["prior_hi"]
   # Small Gaussian ball around the LS best fit, clipped strictly inside priors.
   jitter = np.array([0.10, 5.0, 0.05, 0.20, 0.05])
   p0 = theta_hat + jitter * np.random.randn(MCMC_N_WALKERS, ndim)
   p0 = np.clip(p0, lo + 1e-9, hi - 1e-9)
   sampler = emcee.EnsembleSampler(
      MCMC_N_WALKERS, ndim, _mcmc_log_prob, args=(data,), pool=_SENS_POOL,
   )
   sampler.run_mcmc(p0, MCMC_N_STEPS, progress=False)
   chain = sampler.get_chain(discard=MCMC_BURN, thin=MCMC_THIN, flat=True)
   disp = chain.copy()
   disp[:, 0] = 10.0 ** disp[:, 0]                      # log10(kappa) -> kappa
   truths_disp = np.array(theta_hat, dtype=float).copy()
   truths_disp[0] = 10.0 ** truths_disp[0]
   fig = corner.corner(
      disp, labels=MCMC_LABELS, truths=truths_disp,
      quantiles=[0.16, 0.5, 0.84], show_titles=True,
      title_fmt=".2g", title_kwargs={"fontsize": 9},
   )
   fig.suptitle(f"{model}: parameter posterior (emcee, {n_samples_label})", fontsize=14)
   fig.savefig(png_path, dpi=150, bbox_inches="tight")
   fig.savefig(pdf_path, bbox_inches="tight")
   plt.close(fig)
   p16, p50, p84 = np.percentile(disp, [16, 50, 84], axis=0)
   return {"p16": p16, "p50": p50, "p84": p84, "chain_disp": disp,
           "acceptance": float(np.mean(sampler.acceptance_fraction))}


def mcmc_constraint_check(model, summary):
   """Print the per-parameter constraint diagnostic (the user's 'is each
   parameter well fit for' check) and return (ok, p16, p50, p84). A parameter is
   'constrained' if its relative 68% width is below MCMC_WIDTH_THRESH; a pair is
   'degenerate' if |Pearson corr| exceeds MCMC_CORR_THRESH (flags kappa-h_ml
   equifinality). Overall PASS requires all constrained, none degenerate, and a
   healthy acceptance fraction."""
   p16, p50, p84 = summary["p16"], summary["p50"], summary["p84"]
   disp, acc = summary["chain_disp"], summary["acceptance"]
   denom = np.where(np.abs(p50) > 0, np.abs(p50), np.nan)
   width = 0.5 * (p84 - p16) / denom
   constrained = width < MCMC_WIDTH_THRESH
   corr = np.corrcoef(disp, rowvar=False)
   n = corr.shape[0]
   max_c, pair = 0.0, (0, 1)
   for i in range(n):
      for j in range(i + 1, n):
         if abs(corr[i, j]) > max_c:
            max_c, pair = abs(corr[i, j]), (i, j)
   degenerate = max_c > MCMC_CORR_THRESH
   acc_ok = 0.15 <= acc <= 0.6
   ok = bool(np.all(constrained)) and not degenerate and acc_ok
   plain = [r"\kappa", "h_ml", "eps", "F_ref", "lambda"]
   print(f"   [corner] {model}: acceptance={acc:.2f}{'' if acc_ok else ' (!)'}  "
         f"=> {'PASS' if ok else 'CHECK'}")
   for k in range(n):
      print(f"       {plain[k]:>7}: p50={p50[k]:.3g}  [{p16[k]:.3g}, {p84[k]:.3g}]  "
            f"relwidth={width[k]:.2f}  {'ok' if constrained[k] else 'WIDE'}")
   print(f"       max|corr|={max_c:.2f} ({plain[pair[0]]}-{plain[pair[1]]})"
         f"{'  DEGENERATE' if degenerate else ''}")
   return ok, p16, p50, p84
...
outdir = Path("./figures_diffusive-eps-corner")
...
for run_type in RUN_TYPES:
   for results in RESULT_KINDS:
      ...
      def make_model_grid_with_ratio(
         ...
         short and sits directly beneath its main axis. All ratio axes share ONE
         y-axis so the panels are directly comparable."""
         nmodels = len(models)
         if nmodels % ncols != 0:
      ...
      param_cols = [
         ...
         # MCMC posterior 16/50/84 percentiles for each of the 5 sampled params.
         f"{name}_{q}"
         for name in ("kappa_pde", "h_ml_pde", "epsilon", "F_ref", "lambda")
      ...
      ensure_dirs(outdir, current_dir, ["step1", "validation", "unblinded", "corner", "budget"])
      ...
      for model in models:
         ...
         for expt in expts:
            ...
            if expt == "piControl":
               ...
               step1_NH_ax_by_model[model] = step1_NH_axs[expt][step1_idx[expt]]  # STEP 2 draws the N-vs-H fit here
               step1_HT_ax_by_model[model] = step1_HT_axs[expt][step1_idx[expt]]  # STEP 2 draws H vs T here
               step1_idx[expt] += 1
         ...
         for expt in expts:
            ...
            if expt == "piControl":
               ...
               def make_pde_dT(F_ref_, T_eq_, t_final_yrs_, eps_=1.0, dz_target_=15.0,
                  ...
                  # curves); the eps-calibration inner loop passes the loosened
                  # SENSITIVITY_* tolerances since its solves only steer the
                  # optimizer, not the final curves.
               ...
               eps = 1.0
               # Inner-loop solves run at the loosened SENSITIVITY_* tolerance --
               # they only steer the alternating calibration; the final reported
               # curves are re-solved tight after convergence below.
               ...
               for eps_iter in range(EPS_MAX_ITER):
                  ...
                  pde_dT_func = make_pde_dT(F_ref, T_eq, t_final_years, eps, dz_target_=PDE_FIT_DZ_TARGET,
                                            rtol_=SENSITIVITY_RTOL, atol_=SENSITIVITY_ATOL)
                  p0 = [min(max(kappa_pde, 1e-7), 1e-3), min(max(h_ml_pde, 1e-3), 300.0)]
                  popt, pcov = curve_fit(
                     pde_dT_func, fit_t, fit_T_abs,
               ...
               # Rebuild the tight-tolerance solver for every downstream curve
               # (pde_T, budget, H_plot); the loop above ran loosened.
               pde_dT_func = make_pde_dT(F_ref, T_eq, t_final_years, eps, dz_target_=PDE_FIT_DZ_TARGET)
               ...
               N_final_line = F_ref - lmbda * T_reg_arr
               N_eps_reg = N_final_line - (eps - 1.0) * H_reg_final
               ...
               # Straight-line fit using the FINAL iterated F_ref/lambda (as
               # opposed to the STEP 1 line already on this axis, which used the
               # standalone Gregory regression's F_ref/lambda).
               ax1.plot(T_reg_arr[order], N_final_line[order], color="tab:orange", lw=2, ls="-.",
                        label=f"Final Fit: F_ref={F_ref:.3f}, -lambda={-lmbda:.3f}")
               ...
               # Ratio of the efficacy-corrected prediction to the plain final
               # straight-line prediction, both using the SAME final F_ref/lambda
               # -- isolates how much the -(eps-1)*H term alone is doing relative
               # to the final Gregory line.
               ...
               # ratio y-axis SHARED across all model panels, one divergent
               # point would blow up the common scale. Mask that thin band to nan.
               denom = np.where(np.abs(N_final_line) > 0.02 * abs(F_ref),
                                N_final_line, np.nan)
               ...
               if abs(eps - 1.0) > 1e-6:
                  ...
                  ax_ht.plot(T_reg_arr[order], H_inv[order], color="green",
               ...
               # ----- MCMC posterior + corner plot (the parameter-fit check) -----
               # Sample the joint posterior of (kappa, h_ml, eps, F_ref, lambda)
               # around the LS best fit and render a corner plot, plus a printed
               # "is each parameter well constrained" diagnostic. Gaussian noise
               # levels: sigma_T from the best-fit temperature residual, sigma_N
               # from the regression RMSE (rmse_eps) computed above.
               if RUN_MCMC_CORNER:
                  T_bestfit = np.asarray(pde_dT_func(fit_t, kappa_pde, h_ml_pde), dtype=float)
                  sigma_T = float(np.sqrt(np.mean((T_bestfit - fit_T_abs) ** 2)))
                  mcmc_data = {
                     "t_fit": np.asarray(fit_t, dtype=float),
                     "T_fit_abs": np.asarray(fit_T_abs, dtype=float),
                     "T_reg": T_reg_arr,
                     "N_reg": np.asarray(N_reg, dtype=float),
                     "reg_years": np.asarray(H_years, dtype=float),
                     "bin_H": bool(bin_H),
                     "t_final_years": float(t_final_years),
                     "sigma_T": max(sigma_T, 1e-6),
                     "sigma_N": max(float(rmse_eps), 1e-6),
                     "prior_lo": MCMC_PRIOR_LO,
                     "prior_hi": MCMC_PRIOR_HI,
                  }
                  theta_hat = np.clip(
                     np.array([np.log10(max(kappa_pde, 1e-7)), h_ml_pde, eps, F_ref, lmbda]),
                     MCMC_PRIOR_LO + 1e-9, MCMC_PRIOR_HI - 1e-9,
                  )
                  n_kept = ((MCMC_N_STEPS - MCMC_BURN) // MCMC_THIN) * MCMC_N_WALKERS
                  corner_summary = run_mcmc_corner(
                     theta_hat, mcmc_data, model, f"{n_kept} samples",
                     outdir / current_dir / "corner" / "png" / f"{model}_posterior_corner{suffix}.png",
                     outdir / current_dir / "corner" / "pdf" / f"{model}_posterior_corner{suffix}.pdf",
                  )
                  _, mc_p16, mc_p50, mc_p84 = mcmc_constraint_check(model, corner_summary)
                  for k, name in enumerate(("kappa_pde", "h_ml_pde", "epsilon", "F_ref", "lambda")):
                     df.loc[df["model"] == model, f"{name}_p16"] = mc_p16[k]
                     df.loc[df["model"] == model, f"{name}_p50"] = mc_p50[k]
                     df.loc[df["model"] == model, f"{name}_p84"] = mc_p84[k]
