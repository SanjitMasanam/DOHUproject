...
# --- Joint nonlinear fit (replaces the alternating eps/kappa/h_ml loop) --------
# Instead of alternating a radiative regression with a kappa/h_ml refit (two
# DIFFERENT objectives, no joint descent guarantee -> can limit-cycle and return
# an arbitrary iterate), fit all five parameters at once by minimizing a single
# stacked residual: the AOGCM temperature timeseries AND the Net-TOA radiative
# balance, each weighted by its own noise. Parameters are
#   theta = [log10(kappa), h_ml, eps, lambda]
# (log10 kappa for scale). T_eq is NOT a free parameter: it is CONSTRAINED to the
# value found in STEP 1 (the Gregory regression), i.e. F_ref/lambda = T_eq(step1)
# is enforced by deriving F0 = lambda*T_eq_fixed each evaluation. This both breaks
# the F-lambda ridge and pins the equilibrium warming to the well-constrained
# large-scale Gregory estimate instead of letting it drift. The forward model is
# ONE PDE solve per residual evaluation (returns both T(t) and H), and the
# finite-difference Jacobian columns are farmed out across _SENS_POOL, so a full
# TRF step costs ~1 serial solve of wall-clock time.
RUN_JOINT_FIT = True
...
def _joint_forward(theta, T_eq, fit_t, H_years, bin_H, t_final_years):
   """One PDE solve for the joint fit; returns (T_model at fit_t, H at the
   regression years, 50-yr-binned iff bin_H). theta = [log10(kappa), h_ml, eps,
   lambda]; T_eq is the FIXED STEP-1 value and F0 = lambda*T_eq (enforcing the
   F/lambda = T_eq(step1) constraint). Loosened solver tolerance (SENSITIVITY_*),
   since this feeds a least-squares residual, not the final reported curve.
   Returns None on a failed/non-finite solve so the caller can substitute a
   large residual and steer the optimizer away."""
   log10_kappa, h_ml, eps, lam = theta
...
def _joint_residuals(theta, data):
   ...
   Module-level so the parallel Jacobian can pool.starmap it."""
   out = _joint_forward(theta, data["T_eq"], data["fit_t"], data["H_years"],
                        data["bin_H"], data["t_final_years"])
   if out is None:
      return data["big"]
   T_model, H_reg = out
   eps, lam = theta[2], theta[3]
   T_eq = data["T_eq"]
...
def run_joint_fit(theta0, data):
   """Bounded TRF least-squares over theta = [log10(kappa), h_ml, eps, lambda]
   against the stacked temperature+radiative residual, with the parallel
   Jacobian. T_eq is held FIXED at data["T_eq"] (the STEP-1 Gregory value), so
   F_ref = lambda*T_eq enforces the F/lambda = T_eq(step1) constraint. Returns a
   summary dict: fitted params + 1-sigma errors (from the Jacobian covariance),
   physical-unit RMSEs, convergence status, and the identifiability/bound
   diagnostics used by joint_fit_check."""
   ...
   theta = sol.x
   log10_kappa, h_ml, eps, lam = theta
   T_eq = data["T_eq"]                # fixed at STEP-1 value (not fitted)
   kappa = 10.0 ** log10_kappa
   ...
   errs = np.array([kappa_err, perr[1], perr[2], perr[3]])
   ...
   relwidth = np.array([
      ...
      perr[2] / max(abs(eps), 1e-9),
      perr[3] / max(abs(lam), 1e-9),
   ])
   ...
   return {
      ...
      "F_ref": lam * T_eq,
      "kappa_err": kappa_err, "h_ml_err": perr[1], "eps_err": perr[2],
      "T_eq_err": 0.0, "lam_err": perr[3],
      ...
      for model in models:
         ...
         for expt in expts:
            ...
            if expt == "piControl":
               ...
               # ----- Joint nonlinear fit of (kappa, h_ml, eps, lambda) --------
               # (T_eq held fixed at the STEP-1 value: F/lambda = T_eq(step1))
               # One bounded TRF least-squares over a single stacked residual
               # (AOGCM temperature timeseries + Net-TOA radiative balance),
               # replacing the alternating regression/refit loop (which optimizes
               # two different objectives and can limit-cycle). See run_joint_fit
               # and the JOINT_* constants above.
               ra = reg_arrays[model]
               ...
               sigma_N = max(5e-2, float(np.sqrt(np.mean(
                  (N_reg - (F_ref - lmbda * T_reg)) ** 2))))

               joint_data = {
                  "fit_t": np.asarray(fit_t, float), "T_obs": np.asarray(fit_T_abs, float),
                  "T_reg": T_reg, "N_obs": N_reg,
                  "T_eq": float(T_eq_seed),            # STEP-1 T_eq, held fixed in the fit
                  "H_years": np.asarray(H_years, float), "bin_H": bool(bin_H),
               ...
               # Adopt the joint-fit parameters everywhere downstream. T_eq stays
               # at its STEP-1 value (F_ref = lambda*T_eq enforces F/lambda=T_eq).
               kappa_pde, h_ml_pde = joint["kappa"], joint["h_ml"]
               eps, T_eq, lmbda, F_ref = joint["eps"], joint["T_eq"], joint["lam"], joint["F_ref"]
               eps_unc = joint["eps_err"]
               ...
               print(f"   [joint] {model}: eps={eps:.3f}+/-{eps_unc:.3f} "
                     f"kappa={kappa_pde:.2e} h_ml={h_ml_pde:.0f} F={F_ref:.2f} "
                     f"lam={lmbda:.2f} T_eq={T_eq:.2f} (fixed at STEP-1)")

               # T_eq is fixed at STEP 1, but lambda was refit, so F_ref = lambda*T_eq
               # was updated; refresh the df row (ratio arrays are unchanged since
               # T_eq did not move).
               ...
               df.loc[df["model"] == model, "T_eq_unc"] = joint["T_eq_err"]
               df.loc[df["model"] == model, "lambda_unc"] = joint["lam_err"]
